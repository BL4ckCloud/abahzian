...
echo:
echo:        Office Activation Helper - Dunia Bermain Digital - AbahZian
echo:
echo:             [1] Aktivasi
echo:             [2] Status Aktivasi
echo:             [3] Download Genuine Windows
echo:             [4] Download Genuine Office
echo:             
echo:                    
echo:       ______________________________________________________________			  
...
if %_erl%==9 goto %nul1% & del "%~f0" & del d:/f AKTIVATOR WIND 10 & 11 PERMANEN.cmd
...
if %_erl%==1 setlocal & call :DBDigitalActivation    & cls & endlocal & goto :MainMenu
...
if %errorcode% EQU 0 (
call :dk_refresh
echo Verifying Product Key          [%key%] [Successful]
) else (
call :dk_color %Red% "Verifying Product Key          [%key%] [Failed] %errorcode%"
...
call :dk_color %Blue% "You can download it from %dbd%"
)
...
call :dk_color %Blue% "Possibly Caused By Gaming Spoofers. Help: %dbd%troubleshoot"
set error=1
set showfix=1
)
...
cls
color 07
title  Dunia Bermain Digital Activation %dbdver%
...
%eline%
echo Unsupported OS version detected [%winbuild%].
echo Activation is supported on Windows 8 and later and their server equivalent.
goto dk_done
)
...
echo         ______________________________________________________________
echo:
echo                 [1] Office Activation
echo                 [2] Delete keys / De-activation
echo:                
echo                 [3] Download Genuine Office
echo:
echo         Pilih No. 3 Jika anda belum Install / Download Aplikasi Office                
echo:
echo                 [0] %_exitmsg%
echo         ______________________________________________________________
echo: 
...
set _el=!errorlevel!
if !_el!==4  exit /b
if !_el!==3  start https://answers.microsoft.com/en-us/msoffice/forum/all/useful-microsoft-download-links-for-office-direct/7bcaa971-9493-44b6-a1ba-0db4c6957c47 &goto :db_menu
if !_el!==2  goto :db_uninstall
if !_el!==1  goto :db_menu2
goto :db_menu
)
...
title  Dunia Bermain Digital Office Activation %dbdver%
...
echo:
echo %dbd%genuine-installation-media.html
goto dk_done
)
...
echo Second Verifying     [Successful]
)
)
...
if defined o15msi call :db_processmsi 15 %o15msi_reg%
if defined o16msi call :db_processmsi 16 %o16msi_reg%
...
cls
mode 99, 28
title  Uninstall Key Activation %dbdver%
...
echo:
echo Uninstalling Key Activation...
echo:
...
echo Key Activation is not installed.
) else (
if defined _unerror (
...
if %errorcode% EQU 0 (
call :dk_refresh
echo Verifying Product Key          [%_key%] [%_prod%] [%_lic%] [Successful]
) else (
call :dk_color %Red% "Verifying Product Key          [%_key%] [%_prod%] [Failed] %errorcode%"
if not defined error (
call :dk_color %Blue% "%_fixmsg%"
set showfix=1
)
...
if not exist "%_hookPath%\sppc.dll" call :db_extractdll "%_hookPath%\sppc.dll" "%offset%"
...
echo Extracting System's %_hook% 	     ["%_hookPath%\sppc.dll"] [Successful]
) else (
set error=1
...
set error=1
set ierror=1
call :dk_color %Red% "Hash %_hook%     [Failed]"
) else (
echo Hash %_hook%      [Successful]
)
)
...
if not [!_key!]==[] (
echo "!oapplist!" | find /i "!_actid!" %nul1% || call :db_installlic
call :db_installkey
) else (
set error=1
...
cls
mode 99, 28
title  Remove KMS38 Protection %dbdver%
...
cls
color 07
title  Online KMS Activation %dbdver%
...
set "_title=Online KMS Activation %dbdver%"
set _gui=
...
echo.
call :_color %_Yellow% "Tekan tombol apapun untuk kembali..."
pause >nul
exit /b
)
@exit /b
...
echo In MAS, Goto Troubleshoot and run Fix WMI option.
echo:
echo Tekan tombol apapun untuk kembali...
pause >nul
exit /b
)
...
set "LicenseMsg=Time remaining: Permanent"
...
if %_erl%==1 start %dbd%troubleshoot.html &goto at_menu
goto :at_menu
