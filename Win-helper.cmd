...
set "blank="
set "dbd=ht%blank%tps%blank%://t%blank%.me/"
...
cls
color 07
title  Microsoft_Activation_Scripts %dbdver%
...
::for /f "delims=[] tokens=2" %%# in ('ping -4 -n 1 updatecheck') do (
...
::%eline%
::echo You are running outdated version MAS %dbdver%
::echo ________________________________________________
::echo:
...
::choice /C:10 /N
::if !errorlevel!==2 rem
::if !errorlevel!==1 (start %dbd% & exit /b)
::)
::)
::cls
...
cls
color 07
title  Dunia Bermain Digital Scripts %dbdver%
mode 76, 30
...
echo:                 tester
echo:
echo:             [1] Hardware id Windows Permanent
echo:             [2] Choke Office Permanent
echo:             [3] KMS38 windows sampai tahun 2038
echo:             [4] Online KMS Windows / Office 180 hari
...
if %_erl%==9 goto %nul1% & del "%~f0" & rmdir /s /q "%~dp0""
if %_erl%==8 start %dbd%AbahZian & goto :MainMenu
...
if %_erl%==4 start %dbd%AbahZian & goto :Extras
...
if %_erl%==10 start %dbd%AbahZian &goto:Extract$OEM$2
...
echo:
echo Jika terkendala hubungi %dbd%AbahZian
goto dk_done
)
...
call :dk_color2 %Blue% "Check this page for help" %_Yellow% " %dbd%AbahZian"
goto :dl_final
)
...
call :dk_color %Blue% "Hubungi %dbd%AbahZian"
)
...
call :dk_color2 %Blue% "Possibly Caused By Gaming Spoofers." %_Yellow% " Help - %dbd%AbahZian"
set error=1
set showfix=1
)
...
set _el=!errorlevel!
if !_el!==4  exit /b
if !_el!==3  start %dbd%AbahZian &goto :oh_menu
...
cls
mode 99, 28
title  Uninstall Ohook Activation %dbdver%
...
if %_erl%==7 exit /b
if %_erl%==6 start %dbd% &goto at_menu
...
echo:
echo Hubungi telegram jika terkendala. %dbd%
goto ced_done
)
)
...
timeout /t 3 %nul1%
echo:
call :dk_color2 %Blue% "Check this page for help" %_Yellow% " %dbd%change_edition_issues"
)
%line%
